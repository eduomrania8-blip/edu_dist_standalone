...
-- 2. جدول المدارس الأساسية (التي يعمل بها المعلمون وتتم متابعتها)
CREATE TABLE IF NOT EXISTS base_schools (
    id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
    school_code TEXT UNIQUE,
    school_name TEXT NOT NULL,
    stage TEXT NOT NULL,
    school_type TEXT NOT NULL,
    administration TEXT DEFAULT 'العمرانية',
    created_at TIMESTAMPTZ DEFAULT NOW()
);

-- 3. جدول المدارس السنوية (المتابعة الدورية للموجهين)
CREATE TABLE IF NOT EXISTS supervisor_annual_schools (
    id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
    supervisor_id UUID REFERENCES supervisors(id) ON DELETE CASCADE,
    base_school_id UUID REFERENCES base_schools(id) ON DELETE CASCADE,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    -- الموجه لا يمكن أن يُخصص لنفس المدرسة أكثر من مرة في المتابعة
    UNIQUE(supervisor_id, base_school_id)
);

-- 4. إضافة سياسات الأمان RLS
ALTER TABLE base_schools ENABLE ROW LEVEL SECURITY;
ALTER TABLE supervisor_annual_schools ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Enable read access for all authenticated users"
    ON base_schools FOR SELECT TO authenticated USING (true);
CREATE POLICY "Enable all actions for admin"
    ON base_schools FOR ALL TO authenticated
    USING ((auth.jwt() ->> 'role') = 'admin' OR (auth.jwt() ->> 'email') = 'admin@edu.local');

CREATE POLICY "Enable read access for all authenticated users"
    ON supervisor_annual_schools FOR SELECT TO authenticated USING (true);
CREATE POLICY "Enable all actions for admin"
    ON supervisor_annual_schools FOR ALL TO authenticated
    USING ((auth.jwt() ->> 'role') = 'admin' OR (auth.jwt() ->> 'email') = 'admin@edu.local');
